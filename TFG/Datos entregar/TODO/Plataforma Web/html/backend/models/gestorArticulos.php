...
class GestorArticulosModel{
	...
	public function guardarArticuloModel($datosModel, $tabla){
		...
		$stmt = Conexion::conectar()->prepare("INSERT INTO $tabla (titulo, introduccion, ruta, contenido, id_user) VALUES (:titulo, :introduccion, :ruta, :contenido, :iduser)");
		...
		$stmt -> bindParam(":iduser", $datosModel["usuario"], PDO::PARAM_INT);
	...
	public function mostrarArticulosModel($tabla){
		...
		$stmt = Conexion::conectar()->prepare("SELECT id, titulo, introduccion, ruta, contenido FROM $tabla ORDER BY orden ASC, id DESC");
	...
	public function editarArticuloModel($datosModel, $tabla){
		$stmt = Conexion::conectar()->prepare("UPDATE $tabla SET titulo = :titulo, introduccion = :introduccion, ruta = :ruta, contenido = :contenido, id_user=:iduser WHERE id = :id");	
